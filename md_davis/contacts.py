...
@click.command(name='contacts', context_settings=CONTEXT_SETTINGS)
@click.option('-x', '--xpm_file', metavar='<.xpm>',
              help='Contact/H-bond file obtained from GROMACS')
@click.option('-i', '--index_file', metavar='<.ndx>', help='Index file')
@click.option('-s', '--structure', metavar='<.pdb/.gro>',
              help='Structure file')
@click.option('-g', '--group', metavar='<string>', help='Group to match '
              'from index file to get the list of Contacts/H-bonds')
@click.option('-o', '--output', metavar='<.p>', help='Output Pickle file')
@click.option('--hbonds/--contacts', default=False,
